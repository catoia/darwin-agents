...
9. Commit             → git add -A && git commit -m "[{{id}}] <description>" && git push
```

**Never end a session without committing.** Always prefix commit messages with `[{{id}}]` so the God Agent can track which project made which changes. If you did nothing meaningful, write a note
